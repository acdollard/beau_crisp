...
`);
  return renderTemplate`${renderComponent($$result, "Layout", $$Layout, { "title": "Workout Plans - Beau Crisp" }, { "default": async ($$result2) => renderTemplate` ${maybeRenderHead()}<div class="max-w-4xl mx-auto min-h-[96vh] pt-48"> <div class="flex justify-between items-center mb-8"> <h1 class="text-4xl font-bold">Workout Plans</h1> <a href="/workouts/new" class="bg-ebony text-white px-4 py-2 hover:bg-ebony rounded-sm">
Create New Plan
</a> </div> <div class="grid gap-6"> ${workoutPlans.map((plan) => renderTemplate`<div class="bg-white p-6 rounded-sm shadow"> <div class="flex justify-between items-start"> <div> <h2 class="text-2xl font-bold mb-2"> <a${addAttribute(`/workouts/${plan.slug.current}`, "href")} class="hover:text-blue-600"> ${plan.title} </a> </h2> <p class="text-gray-600 mb-2">Client: ${plan.client?.name}</p> <p class="text-gray-600 mb-4"> ${new Date(plan.startDate).toLocaleDateString()} -
...
}, "/Users/alexanderdollard/Development/b2/frontEnd/src/pages/workouts.astro", void 0);

const $$file = "/Users/alexanderdollard/Development/b2/frontEnd/src/pages/workouts.astro";
const $$url = "/workouts";
