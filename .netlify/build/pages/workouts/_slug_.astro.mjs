...
${workoutPlan.endDate ? new Date(workoutPlan.endDate).toLocaleDateString() : "Ongoing"} </p> ${workoutPlan.description && renderTemplate`<p class="mt-4 text-gray-700">${workoutPlan.description}</p>`} </div> <div class="space-y-8"> ${workoutPlan.workouts?.map((workout) => renderTemplate`<div class="bg-white p-6 rounded-lg shadow"> <h2 class="text-2xl font-bold mb-4">${workout.day}</h2> <div class="space-y-4"> ${workout.exercises?.map((exercise) => renderTemplate`<div class="border-b pb-4 last:border-0"> <h3 class="font-semibold text-lg">${exercise.exercise}</h3> <p class="text-gray-600"> ${exercise.sets} sets × ${exercise.reps} </p> ${exercise.notes && renderTemplate`<p class="text-gray-500 mt-2 text-sm">${exercise.notes}</p>`} </div>`)} </div> </div>`)} </div> </div> ` })}`;
}, "/Users/alexanderdollard/Development/Beau_Crisp/src/pages/workouts/[slug].astro", void 0);

const $$file = "/Users/alexanderdollard/Development/Beau_Crisp/src/pages/workouts/[slug].astro";
